...
def read_data():
    data_list = []
    data_path = './data/uie_data.json'
    with open(data_path, 'r') as fr:
        for line in fr.readlines():
            data = eval(line)
            data_list.append(data)
    return data_list
def process_supplier_json(input_json):
    output_json = []

    for supplier_info in input_json:
        supplier_name_list = supplier_info.get("供应商名称", [])
        processed_supplier_list = []

        for supplier_name in supplier_name_list:
            probability = supplier_name.get("probability", 0)

            if probability > 0.4:
                supplier_name_text = supplier_name.get("text", "")

                # 构建中标信息的 JSON
                bid_info_json = {"中标金额": None, "中标单位": supplier_name_text}

                # 检查是否有中标金额关联
                relations = supplier_name.get("relations", {})
                bid_amount_list = relations.get("中标（成交）金额", [])

                if bid_amount_list:
                    bid_amount_info = bid_amount_list[0]
                    bid_info_json["中标金额"] =  process_bid_amount(bid_amount_info.get("text", ""))
                else:
                    bid_info_json["中标金额"]="null"

                processed_supplier_list.append(bid_info_json)

        # 添加到输出 JSON
        output_json.extend(processed_supplier_list)

    return output_json
def process_bid_amount(bid_amount_text):
    # 使用正则表达式提取中标金额中的数字部分（包括小数点）
    match = re.search(r'(\d+(\.\d+)?)', bid_amount_text)
    if match:
        amount_value = match.group(1)
        # 判断是否包含“万”中文字符
        if "万" in bid_amount_text:
            # 将数字部分乘以10000
            amount_value = float(amount_value) * 10000
...
def process_json(input_json):
    ...
    for key, candidates in input_json[0].items():
        if key == "评委名单":
            pw_li=[]
            # 对于评委名单，将所有的text都放入列表，然后放在评委公司:[]一类的格式中
            for candidate in candidates:
                pw_li.append(candidate["text"])
            output_json[key] =pw_li
        else:
            # 对于其他键，选择probability最高的candidate
            selected_candidate = max(candidates, key=lambda x: x["probability"])
            output_json[key] = selected_candidate["text"]

    return output_json
if __name__ == '__main__':
    args = parse_args()
    args.schema = ['代理机构','评委名单','采购方式','采购单位','采购单位联系地址','采购单位联系电话','所属地区','采购单位联系人姓名','项目编号']
    args.schema_lang = "ch"
    uie = UIEPredictor(model=args.model, task_path=args.task_path, schema_lang=args.schema_lang, schema=args.schema, engine=args.engine, device=args.device,position_prob=args.position_prob, max_seq_len=args.max_seq_len, batch_size=64, split_sentence=False, use_fp16=True)
    datas=read_data()
    
    result_path = './output/result_500.txt'
    with open(result_path, 'w', encoding='utf-8') as fw:
        for i, data in enumerate(datas):
            if i % 10 == 0:
                print(i)
            uie.set_schema(['代理机构','评委名单','采购方式','采购单位','采购单位联系地址','采购单位联系电话','所属地区','采购单位联系人姓名','项目编号'])
            response=process_json(json.loads(str(uie(data['input'])).replace("'", "\"")))
            schema={
  "供应商名称": [
    "中标（成交）金额"
  ]
            ...
            result_dir={}
            response['中标单位']=process_supplier_json(json.loads(str(uie(data['input'])).replace("'", "\"")))
